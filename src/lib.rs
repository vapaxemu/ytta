...
fn link(_: Request, cx: RouteContext<Config>) -> Result<Response> {
    ...
    let ss_link = format!("ss://{}@{host}:443?plugin=v2ray-plugin%3Btls%3Bmux%3D0%3Bmode%3Dwebsocket%3Bpath%3D%2FID%3Bhost%3D{host}#Jatim ss", URL_SAFE.encode(format!("none:{uuid}")));
